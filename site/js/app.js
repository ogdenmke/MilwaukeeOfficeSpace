...
function initMap(buildings) {
  ...
  if (legendEl) {
    ...
    Object.values(groups).forEach((buildings) => {
      ...
      num++;
      if (buildings.length === 1) {
        legendEl.innerHTML += `<a class="map-legend-item" href="building.html?id=${first.building_id}"><span class="map-legend-num">${num}</span><span class="map-legend-name">${escapeHtml(first.building_name)}</span></a>`;
      } else {
        buildings.forEach((b, j) => {
          legendEl.innerHTML += `<a class="map-legend-item" href="building.html?id=${b.building_id}"><span class="map-legend-num" ${j > 0 ? 'style="visibility:hidden"' : ''}>${num}</span><span class="map-legend-name">${escapeHtml(b.building_name)}</span></a>`;
        });
      }
    });
  }
